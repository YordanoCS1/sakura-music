...
import { Search, Youtube, Clock, User, Eye, Download, TrendingUp, Loader2 } from 'lucide-react';
...
export const SearchPage: React.FC = () => {
  ...
  const loadTrending = async () => { setTrending(await invoke<SearchResult[]>('get_trending')); };
  ...
  const handleUseInDownloader = () => {
    if (selectedResult) {
      localStorage.setItem('pendingUrl', selectedResult.url); localStorage.setItem('pendingTitle', selectedResult.title);
      toast.success('URL lista para descargar'); window.dispatchEvent(new CustomEvent('url-ready', { detail: selectedResult.url }));
    }
  };
              ...
              <div className="flex gap-3 p-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex flex-wrap gap-x-3 gap-y-1 text-[11px]" style={{ color: 'var(--text-label)' }}>
                    ...
                    {result.duration && <span className="flex items-center gap-1"><Clock size={11} />{fmtDur(result.duration)}</span>}
                    {result.view_count && <span className="flex items-center gap-1"><Eye size={11} />{fmtViews(result.view_count)}</span>}
                  </div>
                </div>
                {selectedResult?.id === result.id && (
                  <motion.button initial={{ scale: 0 }} animate={{ scale: 1 }}
                    onClick={e => { e.stopPropagation(); handleUseInDownloader(); }}
                    style={{ padding: '6px 14px', borderRadius: 8, border: 'none', background: 'oklch(var(--zen-sakura-base))', color: 'white', fontSize: '0.75rem', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5, cursor: 'pointer', alignSelf: 'center', whiteSpace: 'nowrap' }}>
        ...
        <motion.div initial="hidden" animate="show" variants={{ hidden: {}, show: { transition: { staggerChildren: 0.05 } } }} className="space-y-3">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {trending.slice(0, 8).map((result, idx) => (
              <motion.div key={result.id} variants={itemAnim} whileHover={{ y: -1 }} onClick={() => setSelectedResult(result)}
                style={{ background: 'var(--bg-card)', border: 'var(--border-card)', borderRadius: 10, cursor: 'pointer', padding: 10 }}>
                <div className="flex gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {result.duration && <div className="text-[10px] mt-0.5" style={{ color: 'var(--text-dim)' }}>{fmtDur(result.duration)}</div>}
                  </div>
                </div>
              </motion.div>
            ))}
